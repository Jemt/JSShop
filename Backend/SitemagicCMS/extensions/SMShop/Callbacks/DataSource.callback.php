...
function SMShopGetJsonData()
{
	...
	$json = json_decode($data, true); // XMLHttpRequest always sends UTF-8 which is also what json_decode(..) expects
	$json = SMShopDecodeArrayFromUtf8ToLatin1($json); // Convert data to ISO-8859-1
	return $json;
}
...
function SMShopDataItemToJson($dsDef, $props, SMKeyValueCollection $item)
{
	...
	{
		$res .= (($res !== "") ? ", " : "") . "\"" . $prop . "\": " . (($dsDef["Fields"][$prop]["DataType"] === "string") ? "\"" . SMShopEscapeJsonValue($item[$prop]) . "\"" : (($item[$prop] !== "") ? $item[$prop] : "0"));
	}

	return utf8_encode("{" . $res . "}");
}

function SMShopEscapeJsonValue($str)
{
	$str = str_replace("\\", "\\\\", $str);
